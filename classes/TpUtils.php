...
class TpUtils
{
	...
	static function MakeTag( $nsPrefix, $elementName, $value, $indent = '', $attrs = array() )
	{
		...
		$s = sprintf( '%s<%s%s%s%s>', $indent, $nsPrefix, $ns_sep, $elementName, $xml_attrs );
	...
	static function LoadLibrary( $libName )
	{
		...
		if ( ! extension_loaded( $libName ) ) {
			// "La librairie '$libName' n'est pas installée sur ce serveur. Veuillez l'installer pour utiliser TapirLink.";
			echo "The '$libName' library is not installed on this server. Please, install it to use Tapirlink.";
		}
