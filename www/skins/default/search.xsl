...
<xsl:template match="/">
  ...
  <body>
    ...
    <xsl:if test="$next >= 0">
    <br/><a href="{$accesspoint}?op=s&amp;xslt={$base-url}skins/{$skin}/search.xsl&amp;s={$next}&amp;l=20&amp;cnt=1&amp;e=1&amp;f={$concept-id}%20equals%20%22{$value}%22&amp;m={$base-url}skins/{$skin}/model.xml.php%3Fc%3D{$concept-id}%26v%3D{$value}%26a%3D{$accesspoint}">next</a>
    </xsl:if>
  </body>
  </html>
</xsl:template>
